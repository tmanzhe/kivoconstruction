...
function createBeam(width: number, height: number, index: number): Beam {
  // Alternate between red and blue hues
  const hues = [210, 350]; // 210 = blue, 350 = red
  const hue = hues[index % hues.length] + Math.random() * 10 - 5;
...
export default function BeamsBackground({ className, intensity = "strong", children }: AnimatedGradientBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const updateCanvasSize = () => {
      ...
      beamsRef.current = Array.from({ length: totalBeams }, (_, i) => createBeam(canvas.width, canvas.height, i))
    }
  ...
  return (
    <div className={cn("relative min-h-screen w-full overflow-hidden bg-neutral-800", className)}>
      ...
      <div className="relative z-10 flex min-h-screen w-full items-center justify-center p-4 py-16 md:py-32">
        {children}
      </div>
    </div>
  )
